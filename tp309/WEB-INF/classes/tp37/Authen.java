...
public class Authen extends HttpServlet {
	public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			...
			String query = "select * from users where login='" + req.getParameter("login") + "' and mdp='"
			...
			if (rs.next()) {
				...
				// les autres pages devront tester la presence de login pour savoir si on a bien
				// ete authentifie
				Personne p = new Personne(rs.getString("login"), rs.getString("mdp"), rs.getString("nom"),
						rs.getString("prenom"), rs.getString("role"), rs.getString("adresse"));
				...
				con.close();
				out.println(
						"<a href='../login.html'><button type='button' class='btn btn-default btn-lg'>Retour</button></a>");
				out.println("</div>");
				out.println("</div>");
				out.println("</div>");
			}
			...
			out.println("<div class='alert alert-warning' role='alert'>Erreur " + e.getClass() + " : " + e.getMessage()
					+ "</div>");
		} finally {
			try {
				con.close();
